...
from morpho_cc import MorphoCC
...
def main():
    ...
    # --- Morpho (Project LYDIA) timbre layer ---------------------------
    # Optional: if --morpho-out isn't given, tension is still computed
    # (cheap) and printed with --morpho-log, but nothing is sent over MIDI.
    # This lets tension.py get tuned by ear from the console log before
    # any Morpho hardware is actually connected.
    ap.add_argument("--morpho-out", help="MIDI output port name for the Neutone Morpho unit "
                                          "(separate from --midi-out; omit to run without it)")
    ap.add_argument("--morpho-cc", type=int, default=1,
                     help="CC number Morpho's blend/morph parameter listens on -- confirm the "
                          "real value against Project LYDIA's docs or the Roland/Neutone team, "
                          "this default (mod wheel, CC1) is a placeholder, not a spec")
    ap.add_argument("--morpho-channel", type=int, default=0)
    ap.add_argument("--morpho-min", type=int, default=0, help="CC value sent at tension=0.0")
    ap.add_argument("--morpho-max", type=int, default=127, help="CC value sent at tension=1.0")
    ap.add_argument("--morpho-smoothing", type=float, default=0.3,
                     help="0-1, higher = snappier CC response to a tension jump, lower = "
                          "smoother morph sweeps -- retune by ear")
    ap.add_argument("--tension-window", type=float, default=6.0,
                     help="seconds of recent notes (both voices) the tension score is computed "
                          "over -- see tension.RollingTension")
    ap.add_argument("--morpho-log", action="store_true",
                     help="print every tension update to the console, whether or not "
                          "--morpho-out is given -- use this to tune tension.py by ear/eye "
                          "before Morpho hardware is on hand")
    args = ap.parse_args()
    ...
    morpho_out_name = (
        midi_io.resolve_port(args.morpho_out, ports["outputs"], "output")
        if args.morpho_out else None
    )
    ...
    tension = RollingTension(window_s=args.tension_window)
    morpho = (
        MorphoCC(morpho_out_name, args.morpho_cc, channel=args.morpho_channel,
                 min_val=args.morpho_min, max_val=args.morpho_max, smoothing=args.morpho_smoothing)
        if morpho_out_name else None
    )

    def on_played(onset_s, dur_s, role, pitch):
        # Tension is computed from BOTH voices -- it's a property of the
        # whole texture (how consonant/dissonant, how dense, how wide the
        # leaps are right now), not just the companion's notes -- so update
        # it unconditionally, before the melody/companion split below.
        t = tension.update(onset_s, dur_s, role, pitch)
        if morpho is not None:
            morpho.set_tension(t)
        if args.morpho_log:
            print(f"[tension {t:4.2f}] role={role} pitch={pitch}")
    ...
    if morpho is not None:
        print(f"tension -> Morpho CC{args.morpho_cc} (channel {args.morpho_channel}) on '{morpho_out_name}'")
    elif args.morpho_log:
        print("tension computed and logged, but no --morpho-out given -- nothing sent over MIDI")
